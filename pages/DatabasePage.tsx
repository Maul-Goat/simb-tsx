...
export const getOfficialLandslideData = async (limit?: number): Promise<LandslideFeatureCollection> => {
    const client = getSupabaseClient();
    if (!client) return { type: "FeatureCollection", features: [] };

    let query = client.from('kejadian_longsor').select('*').order('tanggal', { ascending: false });
